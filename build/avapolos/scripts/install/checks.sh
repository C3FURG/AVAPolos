...
echo "checks.sh" | log debug installer
echo "Executando checagens iniciais." | log info installer

#If the system's version is compatible.
if [ "$(lsb_release -rs)" = "18.04" ]; then
  #Tell the user.
  echo "Ubuntu 18.04 é compatível com AVAPolos, continuando..." | log info installer
#If it's not compatible.
else
  #Tell the user and exit with a error code.
  echo "Seu sistema é incompatível com AVAPolos." | log error installer
  exit 1
fi
...
for i in "${services[@]}"; do
  ...
  if $(systemctl is-active --quiet $i); then
    ...
    echo "O servico $i esta rodando, será finalizado" | log info installer
